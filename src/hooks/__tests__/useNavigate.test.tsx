...
describe('useNavigate', () => {
  ...
  it('returns go function that navigates with push', () => {
    ...
    const hook = renderHook(() => useNavigate(), {
      wrapper: Component,
    });

    expect(history.length).toBe(1);

    const navigate = hook.result.current;

    navigate(path).go({ word: 'foo', number: 3 });
    hook.rerender();
  ...
  it('returns set function that navigates with replace', () => {
    ...
    navigate(path).set({ word: 'foo', number: 3 });
    hook.rerender();
  ...
  it('returns set function that navigates with replace and supports query string', () => {
    ...
    navigate(path, query).set({ word: 'foo', number: 3 }, { word: ['a', 'b'] });
    hook.rerender();
  ...
  it('returns on function that returns a callback that navigates with push', () => {
    ...
    const handleSomething = navigate(path).on({ word: 'foo', number: 3 });
    expect(history.length).toBe(1);
  ...
  it('returns to function that returns a string', () => {
    ...
    const navigate = hook.result.current;
    const url = navigate(path).to({ word: 'foo', number: 3 });
  ...
  it('supports path with query', () => {
    ...
    const navigate = hook.result.current;
    const url = navigate(path, query).to(
      { word: 'foo', number: 3 },
  ...
  it('treats query as optional', () => {
    ...
    const navigate = hook.result.current;
    const url = navigate(path, query).to({ word: 'foo', number: 3 });
    expect(url).toEqual('/my/path/foo/3');
  });
